
## 📂 Estructura del Repositorio
- `datos_personales.py`: Programa para registrar información personal.
- `operaciones.py`: Calculadora básica con operaciones aritméticas.
- `conversor.py`: Programa que solicita grados Celsius y devuelve conversion en Fahrenheit y Kelvin.
- `identificacion_de_errores.py`: Programas correjido y explicaciones.
- `calculadora_promedio.py`: Programa que pide el nombre del estudiante asi mismo tambien 3 calificaciones y saca su promedio
-  `Archivos .jpg son evidencias de ejecucion`


## /) /) 
 ( •.• )  
  / >❤️
Pony Creadores!
- Isaac
- Yhosvani
- Ramón

¡Gracias por visitar nuestro proyecto! 🌟  
**#programacionbasicatecnmmorelia**

---

*Última actualización: 09 de Noviembre de 2025*
